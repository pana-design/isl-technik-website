/* Wie _reenc.mjs, aber mit steuerbarer Keyframe-Dichte (KF = alle n Frames
   ein Keyframe). Scroll-Scrubbing braucht dichte Keyframes — sonst muss der
   Browser bei jedem Scrollschritt Dutzende Frames nachdekodieren. */
import { chromium } from 'playwright-core';
import { writeFileSync } from 'fs';
const CH=process.env.HOME+'/Library/Caches/ms-playwright/chromium-1234/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing';
const SRC=process.argv[2], OUT=process.argv[3];
const W=+(process.env.W||900), KBPS=+(process.env.KBPS||900), KF=+(process.env.KF||3);
const TRIM=+(process.env.TRIM||0);
...
const data=await p.evaluate(async ({SRC,W,KBPS,TRIM,KF})=>{
  ...
  const rec=new MediaRecorder(c.captureStream(30),
    {mimeType:'video/mp4;codecs=avc1.4d401f',videoBitsPerSecond:KBPS*1000,videoKeyFrameIntervalCount:KF});
...
console.log(`${OUT}  ${data.w}x${data.h}  ${data.dur.toFixed(2)}s  ${(data.bytes/1024).toFixed(0)} KB  KF=${KF}`);
await b.close();
